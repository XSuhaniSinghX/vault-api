...
// the seventh class
// This class handles the authentication service for the Vault application. It includes methods for user registration and login.
...
public class AuthService {
    ...
    public apiResponse login(String username, String password){// this ones gon be for login
        try{
            user use= FileStorageUtil.load(userbase + username + ".json", user.class);
            if(! encoder.matches(password, use.password))//if name or password doesnt match, lock the sneaky mf out
